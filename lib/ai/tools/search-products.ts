...
const productSearchSchema = z.object({
  query: z
    ...
    .optional()
    .default("")
    .describe("Search term to find products by name, description, or category (e.g., 'oak table', 'leather sofa', 'dining')"),
  category: z
    .string()
    .optional()
    .default("")
  ...
  minPrice: z
    ...
    .optional()
    .default(0)
    .describe("Minimum price in CAD (e.g., 100"),
  maxPrice: z
    .number()
    .optional()
    .default(0)
    .describe("Maximum price in CAD (e.g., 500). Use 0 for no maximum.")
})

export const searchProductsTool = tool({
  description: "Search for products in the furniture store. Can search by name, description, or category, and filter by material, color, and price range. Returns product details including stock availability.",
  ...
  execute: async ({ query, category, material, color, minPrice, maxPrice }) => {
    ...
    try {
      ...
      const formattedProducts: SearchProduct[] = (
        ...
        price: product.price ? formatPrice(product.price) : null,
